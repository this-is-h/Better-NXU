...
2026-09-26 汇总维护分支和团委附件下载功能，脚本版本更新为 2.0.2。团委沿用原有 GM 键与默认关闭值，配置提示版本升至 8；设置页补齐开关保存、数组变更持久化和复选框整行点击。课表 Schema 与加密格式保持不变。

本次 `pnpm check` 的 157 项测试、文档、ESLint、生产构建和元数据校验全部通过；`pnpm test:coverage` 行覆盖率为 87.67%；`pnpm audit --prod --audit-level=high --registry=https://registry.npmjs.org/` 未发现已知漏洞。构建包含 16 个 match、16 个 grant、7 个 require、8 个 resource、5 个 connect；安装脚本为 535,862 字节，gzip 为 138,052 字节。相对上一轮安装版滑块修复增加约 12.5 kB，主要来自团委下载流程、本地 OCR 资源加载与设置修复；约 6.96 MB 的 OCR 资源仍按需下载，不编入安装脚本。

维护者在本次发布准备中确认待发布改动已完成真实 ScriptCat 验证。此前各节记录的独立浏览器测试保留其模拟 GM 的适用范围，不视为本次自动化会话重新执行了 ScriptCat 回归。

本次只提交源码、测试和文档；`dist` 由 PR 合并后的 GitHub Actions 生成并回写。合并后仍需核对 `main` 的 check/publish 结果与最终安装产物版本。
